...
create_traditional_matrix <- function(tst) {
  return(dcast(tst ,patient_num + gp ~ dx_label,length))
}
...
if (verbose) {
  ...
  print(Sys.time())
  print(object.size(tst1))
  print(object.size(tst2))
...
for (ic in i_control) {
  ...
  pc2 <- create_traditional_matrix(lc[lc$gp %in% c(i_case, ic), ])
  iv_matrix <- as.matrix(pc2[,-(1:2)])
  dv_vector <- as.numeric(pc2$gp==i_case)
  en <- glmnet(
