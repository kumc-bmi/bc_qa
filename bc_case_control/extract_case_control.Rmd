...
This section reads in the patient ids from the i2b2 files
...
```{r get_disease_group_databses_names, echo=TRUE}
...
location as the PCORnet CDM database and to the other 
i2b2 variables databases.

```{r write_distinct_patient_numbers}
names(d$st) <- toupper(names(d$st))
dbWriteTable(cdm, "DP", d$st, overwrite=TRUE)
if (verbose) dbListTables(cdm)

for (f in d$vi$file_location) {
  variables_conn <- dbConnect(SQLite(), dbname=f)
  dbWriteTable(variables_conn, "DP", d$st, overwrite=TRUE)
