...
pc <- data.frame(gp=names(pb[1]), pb[[1]], tumor_date=NA, stringsAsFactors=FALSE)
for (k in 2:length(pb)) {
  pc <- rbind(pc, data.frame(gp=names(pb[k]), pb[[k]], tumor_date=NA, stringsAsFactors=FALSE))
}
pc <- rbind(pc, px[ , c("gp", "patient_num", "tumor_date")])
...
ce$dx_label <- paste(strip_specials(ce$short_label), ce$dx_new, "09" sep="_")
...
ie$dx_label <- paste(ie$name_char, ie$concept_cd, ie$abbreviation, sep="_")
...
Almost done. Put the cdm and i2b2 data into a single place.
...
# extract_case_control.RData stores everything, including intermediate files.
