...
d$fi <-
  read.csv(file="disease_group_databases.txt", header=TRUE, stringsAsFactors=FALSE)
...
# Add distinct patient_num, concept_cd
#   and where concept_cd LIKE 'ICD9:%'
# once I am comfortable with this.

sql[["concept_dimension"]] <-
  "select *
...
You should create procedures.db using pretty much the same
methods that created the patient lists, except that you add
in information from the procedures table. I'll elaborate 
more here when I get a chance.


```{r now get all procedure codes, echo=TRUE}

# eventually, I want to use multiple file names here
# for procedures, orders, icd9, icd10, etc. This
# would mean putting the file names in a csv file.

procedures_db <- "../../heron/procedures.db"
procedures_conn <- dbConnect(SQLite(), dbname=procedures_db)

d$pt <- dbListTables(procedures_conn)
notes$pt <- "List of table names from procedures_db"
if (verbose) print(d$pt)

d$of <- dbGetQuery(procedures_conn, sql[["observation_fact"]])
notes$of <- "Data from observation_fact table"

d$cd <- dbGetQuery(procedures_conn, sql[["concept_dimension"]])
notes$of <- "Data from concept_dimension table"

if (verbose) {
  print_random_rows(d$of)
