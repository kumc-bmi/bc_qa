...
Look at tumor_date versus probability.

The plot shows that most of the low probabilities are associated with early tumor dates.

```{r tumor_date_analysis, timer=TRUE}
me <- aggregate(mc$co, mc[, c("patient_num", "GP", "TUMOR_DATE")], sum)
names(me)[4] <- c("co")
me$probability <- update_probability(0.1, me$co)
me$time_interval <- "1938 to 2009"
me$time_interval[me$TUMOR_DATE > as.Date("2010-01-01")] <- "2010 to 2015"
if (qc) {
  print_random_rows(me)
}
archive(me, "aggregated with tumor_date")
plot(me$TUMOR_DATE, me$probability)
```

By time interval, what fraction of breast cancer patients are above the 0.9 threshold?

```{r fraction_above_0.9, timer=TRUE}
tapply(me$probability,me$time_interval, function(x) {round(sum(x>0.9)/length(x>0.9), 2)})
```

By time interval, what fraction of breast cancer patients stay at the baseline probability?

```{r fraction_stay_at_baseline, timer=TRUE}
tapply(me$probability,me$time_interval, function(x) {round(sum(x==0.1)/length(x==0.1), 2)})
```
...
old_ids <- me$patient_num[me$time_interval=="1938 to 2009"] 
mf <- merge(md, data.frame(patient_num=old_ids, exclude=1), all=TRUE)
mf <- mf[is.na(mf$exclude), ]
mg <- split(mf$probability, mf$GP)
probability_list <- sort(unique(mf$probability))
n_p <- length(probability_list)
empty_column <- 
mh <- matrix(NA, nrow=n_p, ncol=length(unique(md$GP)))
# names(mh) <- sort(c(i_case, i_control))
for (i in 1:length(probability_list)) {
  p <- probability_list[i]
  mh[i, ] <- sapply(mg, function(x) {sum(x > p) / length(x)})
}
plot(mh[, 2], mh[, 1], xlim=c(0, 1), ylim=c(0, 1))
```
...
library("chron")
plot_time_course <- function(p) {  
  sb <- mc[mc$patient_num==p, ]
  cumulative_co <- cumsum(sb$co)
  sb$cumulative_probability <- update_probability(0.1, cumulative_co)
  sb$days <- round(as.numeric(sb$observation_date - sb$observation_date[1]) / (24*60*60), 1)
  sb$days[is.na(sb$days)] <- 0
  tumor_day <- as.numeric(sb$TUMOR_DATE - as.Date(sb$observation_date[1]))
  n <- dim(sb)[1]
  par(mfrow=c(1,2))
  par(mar=c(2.6, 2.6, 2.6, 0.6))
  y_start <- min(-30, tumor_day-30, na.rm=TRUE)
  plot(sb$cumulative_probability, sb$days, type="S", xlim=c(0,1), ylim=c(sb$days[n]+30, y_start))
  arrows(0.5, tumor_day[1], 0.1, tumor_day, lwd=3, length=0.1, col="red")
  text(0.55, tumor_day[1], "Registry day", col="red", adj=0)
