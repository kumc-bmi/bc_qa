...
tapply(md$updated_probability,md$GP,function(x) {round(sum(x==0.1)/length(x==0.1), 2)})
```

Now let's peek at random cases.

The codes a bit of a mess because the format for observation_date is not
...
true_positive_patients <-(md$updated_probability > 0.90) & (md$GP==i_case)
for (p in sample(md$patient_num[true_positive_patients], 20)) {
  plot_time_course(p)
}  

false_positive_patients <- (md$updated_probability > 0.90) & (md$GP!=i_case)
for (p in sample(md$patient_num[false_positive_patients], 20)) {
  plot_time_course(p)
}  

false_negative_patients <- (md$co > 0) & (md$updated_probability < 0.90) & (md$GP==i_case)
for (p in sample(md$patient_num[false_negative_patients], 20)) {
  plot_time_course(p)
}  

true_negative_patients <- (md$co > 0) & (md$updated_probability < 0.90) & (md$GP!=i_case)
...
```{r save_important}
save(mc, md, file="evaluate_time_course.RData")
```
