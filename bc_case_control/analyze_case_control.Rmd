...
spec <- round(100*ec[,2:4]/(ec[,2:4]+ex[,2:4]))
...
```{r get_icd9_by_patient, echo=TRUE, eval=FALSE}
library("rpart")
library("reshape2")
library("LogicReg")
...
logic_model <- logreg(resp=as.numeric(pc[,"GP"]=="breastcancer"),
                      bin=pc[,-(1:2)],
                      select=1, ntrees=2,
