...
BCRup.AddVariableToDatamart <- function(p.datamart, p.site.ptobs, 
  ...
  if (tmp.var.found.cnt > 1) {
    ...
    if (p.code.type == "CHAR") {
      # Default and applies to CHAR datatypes
      print(names(tmp.new.col.facts))
      tmp.new.col.facts <- unique(tmp.new.col.facts[,c("patient.num","encounter.num",
                                                       "code","code.name")])
      tmp.new.col.facts$code <- paste0(tmp.new.col.facts$code,"||",tmp.new.col.facts$code.name)
      tmp.new.col.facts <- tmp.new.col.facts[,c("patient.num","encounter.num","code")]
      } 
    else {
      if (p.code.type == "NUM") {
        tmp.new.col.facts <- unique(tmp.new.col.facts[,c("patient.num","encounter.num","nval")])
        } 
      else {
          if (p.code.type == "DATE") {
                  tmp.new.col.facts <- unique(tmp.new.col.facts[,c("patient.num","encounter.num","start.date.char")])
            } 
          else {
                  ...
                  tmp.exception <- data.frame(p.new.col.name,p.code.string.1,p.code.string.2,
                                              tmp.msg,tmp.action.taken)
                  v.exceptions <<- rbind(,v.exceptions,tmp.exception)
          }
      }
            
    }
    tmp.new.col.facts <- setNames(tmp.new.col.facts,c("patient.num","encounter.num",p.new.col.name))
    ...
    message("... number of unique values founds: ",length(unique(tmp.new.col.facts[,p.new.col.name])))
    if (tmp.ncf.cnt == 0) {
...
for (tmp.col.index in 4:(ncol(tmp.tumor.site))) # Skip P#, E# and Site columns
  {  
  BCRup.DescribeDatamartColumn(tmp.tumor.site,tmp.col.index) 
...
# Remove original recept status variables f/ inclusion set, as they are replicated in NAACCR vars
...
message('... ',nrow(csv.tumor.site),' rows written.')
```