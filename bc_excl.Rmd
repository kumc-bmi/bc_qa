...
tumor.site <- v.enc(conn.site,
                    subset(bcterm$term204,
                           ...
                           grepl('0400 Primary Site', concept_path)
                           )$concept_path,
                    'date.dx')
...
tumor.site <- with.var.pat(tumor.site, conn.site,
                           ...
                           'language')
message('@@TODO: re-code UNKNOWN as NA?')
```
...
dem.states <- unique(survey.sample[, c('patient_num', 'female', 'adult', 'not.dead', 'english')])
...
enum.name <- function(x) {
  names <- lapply(x, function(tail) {
    name_char <- dbGetQuery(conn.site,
               "select distinct name_char
               from concept_dimension
               where substr(concept_path, length(concept_path) - length(:tail) + 1) = :tail",
               bind.data=data.frame(tail=tail))
    if (nrow(name_char) < 1) {
      '?'
    } else {
      name_char[1, 1]
...
prop$name <- enum.name(row.names(prop))
...
```{r}
x <- tumor.site[!survey.sample$patient_num %in% check.morph(tumor.site, '8520/2'), ]
