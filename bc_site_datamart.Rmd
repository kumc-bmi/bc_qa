...
message("... ",nrow(tmp.site.datamart)," rows read.")
...
# STAGE 2 - ALL-SITE DATAMART, CONSOLIDATE AND FILTER FOR CONSENTED STUDY IDS

```{r Build consolidated datamart}
v.output.dir <- '/d1/home/vleonardo/GPC-Development/bc-data-files/Phase1-DatamartOutput/'  
v.sites <- c('KUMC','KUMC','MCRF','MCW','UIOWA','UMN','UNMC','UTSW','WISC')
q.all.site.consented.datamart <- BCRup.GetConsentedPtData(v.sites[1])
for (i in v.sites[2:length(v.sites)]) {
  tmp.site.consented.datamart <- BCRup.GetConsentedPtData(v.sites[i])
  q.all.site.consented.datamart <- rbind(q.all.site.consented.datamart,tmp.site.consented.datamart)
  message("... Rows added for ",v.site[i],": ",nrow(tmp.site.consented.datamart))
