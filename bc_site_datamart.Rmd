...
BCRup.VisualizeDatamartColumn <- function(p.datamart,p.col.name) {
     ...
     if  (! tmp.col.found.flag) {
       message("No 'Visual Analysis', as only NAs are present for column: ",tmp.col.name) 
...
conn.site <- dbConnect(SQLite(),paste0('/d1/home/vleonardo/GPC-Development/bc-data-files/Phase1-SourceDbs/',dataset$bc_db,".db") )
...
message(getwd())
v.site.consented.study.ids <- read.csv(paste0("~/GPC-Development/bc-datamart/KUMC-Sample-Consented-StudyIds-2015-09-22.csv"))
v.site.mapping <- read.csv(paste0("~/GPC-Development/bc-datamart/KUMC-study_id_to_order_num.csv"))
