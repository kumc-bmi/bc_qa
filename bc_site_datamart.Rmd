...
v.col.terms <- rbind(v.col.terms, data.frame(term.1='2869', term.2='factor', col.name='NAACCR.2869.HER2.CSSF15', col.data.type='CHAR'))
v.col.terms <- rbind(v.col.terms, data.frame(term.1='2876', term.2='factor', col.name='NAACCR.2876.MS.Method.CSSF22', col.data.type='CHAR'))
v.col.terms <- rbind(v.col.terms, data.frame(term.1='2877', term.2='factor', col.name='NAACCR.2877.MS.Result.CSSF23', col.data.type='CHAR'))
v.col.terms <- rbind(v.col.terms, data.frame(term.1='2880', term.2='factor', col.name='NAACCR.2880.ER.CSSF01', col.data.type='CHAR'))
v.col.terms <- rbind(v.col.terms, data.frame(term.1='2890', term.2='factor', col.name='NAACCR.2890.PR.CSSF02', col.data.type='CHAR'))
...
tmp.sdpc <- setnames(tmp.sdpc,
                     ...
                     new=c("gpc.date.birth.shifted","gpc.date.dx.actl"))
...
v.sites <- c('MCRF')
q.all.site.consented.datamart <- BCRup.GetConsentedPtData(v.sites[1])

q.all.site.consented.datamart <- q.all.site.consented.datamart[order(q.all.site.consented.datamart$gpc.study.id,
                                                                     q.all.site.consented.datamart$encounter.num),]
v.filename <- paste0(v.output.dir,"BCDatamart-Site-Consented-",v.sites[1],".csv")
write.csv(q.all.site.consented.datamart, file = v.filename, row.names=FALSE)
message('Number of rows written: ',nrow(q.all.site.consented.datamart))

}

```

# STAGE 3 - ALL-SITE DATAMART, CONSOLIDATE AND FILTER FOR CONSENTED STUDY IDS
...
v.sites <- c('KUMC','MCRF')
v.filename <- paste0(v.output.dir,"BCDatamart-Site-Consented-",v.sites[1],".csv")
q.all.site.consented.datamart <- read.csv(v.filename)
message("... Rows added for ",v.site[1],": ",nrow(q.all.site.consented.datamart))
for (i in v.sites[2:length(v.sites)]) {
  ...
  q.all.site.consented.datamart <- rbind(q.all.site.consented.datamart,tmp.site.consented.datamart)
  message("... Rows added for ",i,": ",nrow(tmp.site.consented.datamart))
}
...
v.naaccr.code.list <- c('0610','2850','2860','2869','2876','2877','2880','2890','2940','3000','3020','3400','3430')
v.consented.datamart <- read.csv("/d1/home/vleonardo/GPC-Development/bc-data-files/Phase1-DatamartOutput/BCDatamart-AllSite-Consented.csv")
...
if (g.allsites.switch) {
message('GPC Site:                ', dataset$site)
message('Patients loaded:         ',length(unique(q.all.site.consented.datamart$patient.num)))
message('Encounters exported:     ',nrow(q.all.site.consented.datamart),' (num of rows written)')
message('Pts/Encounters eligible: ',length(q.all.site.consented.datamart$gpc.enctr.eligible[q.all.site.consented.datamart$gpc.enctr.eligible]))
message('Dx Age (Mean):           ',mean(unique(q.all.site.consented.datamart[,c("patient.num","gpc.dx.age")])$gpc.dx.age))
message('GPC Birth Date (Min):    ',min(q.all.site.consented.datamart$gpc.date.birth.shifted))
message('NAACCR Birth Date (Min): ',min(q.all.site.consented.datamart$NAACCR.0240.Birth.Date.Shifted))
message('GPC Birth Date (Max):    ',max(q.all.site.consented.datamart$gpc.date.birth.shifted))
message('NAACCR Birth Date (Max): ',max(q.all.site.consented.datamart$NAACCR.0240.Birth.Date.Shifted))
message('GPC Dx Date (Min):       ',min(q.all.site.consented.datamart$gpc.date.dx.shifted))
message('NAACCR Dx Date (Min):    ',min(q.all.site.consented.datamart$NAACCR.0390.Dx.Date.Shifted))
message('GPC Dx Date (Max):       ',max(q.all.site.consented.datamart$gpc.date.dx.shifted))
message('NAACCR Dx Date (Max):    ',max(q.all.site.consented.datamart$NAACCR.0390.Dx.Date.Shifted))
}
```
...
BCRup.Datamart.Stats('ALL SITES',q.all.site.consented.datamart)

# Descriptive Analysis} 
for (tmp.col.index in 4:(ncol(q.all.site.consented.datamart))) # Skip P#, E# and Site columns
  {  
  tmp.col.name <- names(q.all.site.consented.datamart[tmp.col.index])                         
  BCRup.VisualizeDatamartColumn(q.all.site.consented.datamart,tmp.col.name)
  BCRup.DescribeDatamartColumn(q.all.site.consented.datamart, tmp.col.name)    
  }
